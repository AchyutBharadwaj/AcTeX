...
int ccompile(POS *pos, int count, FUNC *def, FILE *fpn, FUNC *token, MAX *max);
...
int main(int argc, char **argv) {
  ...
  max->pos = ret;
  ret = ccompile(pos, max->pos, def, fpn, token, max);
  if(ret == 1) {
    return 0;
  }
...
int ccompile(POS *pos, int count, FUNC *def, FILE *fpn, FUNC *token, MAX *max) {
  ...
  for(i=0; i<count; i++) {
    ...
    else if(huduku(pos[i].command, token, max->token) >= max->token && i != 0) {
